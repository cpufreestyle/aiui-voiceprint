/**
 * 多模态视觉解说适配层（智谱 GLM-4V，OpenAI 兼容 /chat/completions）
 *
 * 平台事实（见 AIUI 文档调研）：
 *   - AIUI 内置 AI 只有「纯文本大模型」session.prompt()，没有视觉/多模态接口；
 *   - 网络能力有 wx.request，但没有 wx.uploadFile。
 * 因此「解说图片/视频内容」走 wx.request POST 到外部多模态网关（默认智谱 GLM-4V）。
 *
 ...
 *
 * 智谱开放平台文档：https://open.bigmodel.cn/dev/api（glm-4v-plus 支持图片与视频关键帧）
 */
...
// 默认配置：预填智谱网关与模型；apiKey 留空（=演示模式），由用户在设置里填入。
// glm-4v-plus 同时支持图片与视频关键帧，比仅支持图片的 glm-4v 更贴合「识别图片和视频」。
